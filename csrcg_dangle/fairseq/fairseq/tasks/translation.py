...
def load_langpair_dataset(
    ...
    print("max src size : "+str(max(src_dataset.sizes)))
    print("max tgt size : "+str(max(tgt_dataset.sizes)))
...
class TranslationTask(FairseqTask):
    ...
    def ana_step(
        ...
        # T B H
        # hs = extra['inner_states'][-1].cpu().numpy()
        # y = sample['target']
        # dictionary = model.decoder.dictionary

        # for il, l in enumerate(y.tolist()):
        #     for iw, wid in enumerate(l):
        #         if wid not in set(range(dictionary.nspecial)):
        #             w = dictionary[wid]
        #
        ...
        #     code_use = np.array([l.availability[k] for k in l.availability])
        #     code_use = code_use / code_use.sum()
        #     print(code_use)
        #
        # exit()
...
@register_task("translation_amp")
class TranslationTaskAMPTask(TranslationTask):
    def __init__(self, args, src_dict, tgt_dict):
