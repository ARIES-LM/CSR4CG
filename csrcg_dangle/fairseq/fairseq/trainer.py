...
class Trainer(object):
    ...
    def __init__(self, args, task, model, criterion, quantizer=None):
        ...
        self._valid_dummy_batch = "DUMMY"  # indicates we don't have a dummy batch at first
    ...
    # added
    def ana_step(self, samples, raise_oom=False):
        ...
        for i, sample in enumerate(samples):
            ...
            # if sample["net_input"]["src_tokens"].size(0) != 5000:
                # continue
            with maybe_no_sync():
                # forward and backward
                loss, sample_size_i, logging_output = self.task.ana_step(
                    sample=sample,
                    model=self.model,
    ...
    def train_step(self, samples, raise_oom=False):
        ...
        try:
            ...
            # print("grad_norm before clip : "+str(self.optimizer.compute_grad_norm()))

            with torch.autograd.profiler.record_function("clip-grads"):
                # clip grads
                grad_norm = self.clip_grad_norm(self.args.clip_norm)

            # print("grad_norm : "+str(grad_norm))
    ...
    def valid_step(self, sample, raise_oom=False):
        ...
        if self._valid_dummy_batch == "DUMMY":
        ...
        with torch.no_grad():
            ...
            sample = self._prepare_sample(sample)
            if sample is None:
                sample = self._prepare_sample(self._valid_dummy_batch)
                is_dummy_batch = True
            else:
                is_dummy_batch = False
