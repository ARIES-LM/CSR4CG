...
def main(args):
    ...
    if args.results_path is not None:
        ...
        sample_output_path = os.path.join(args.results_path, 'generate-sample-{}.txt'.format(args.gen_subset))
        with open(output_path, 'w', buffering=1, encoding='utf-8') as h, open(sample_output_path, 'w', buffering=1, encoding='utf-8') as h_sample:
            return _main(args, h, h_sample)
    else:
        return _main(args, sys.stdout)


def _main(args, output_file, sample_output_file=None):
    logging.basicConfig(
    ...
    for sample in progress:
        ...
        for i, sample_id in enumerate(sample['id'].tolist()):
            ...
            if align_dict is not None:
                ...
                #have to retrieve the orignial target sentences for the test set because the target tokens are not tokenized right
                if args.eval_cognition:
                    target_str = task.dataset(args.gen_subset).tgt.get_original_text(sample_id)
            ...
            for j, hypo in enumerate(hypos[i][:args.nbest]):
                ...
                if args.eval_cognition:
                    translations[sample_id] = detok_hypo_str
                    target_str = " ".join(list(target_str.replace(" ","")))
                    detok_hypo_str = " ".join(list(detok_hypo_str.replace(" ","")))
    ...
    for i in range(len(translations)):
        ...
        if sample_output_file is not None:
            print(translation, file=sample_output_file)
    ...
    if has_target:
        ...
        if args.eval_cognition:
            ...
            # added
            # logger.info('Generate {} with beam={}: {}'.format(args.gen_subset, args.beam, scorer.result_string(tokenize='none')))
            logger.info('Generate {} with beam={}: {}'.format(args.gen_subset, args.beam, scorer.score()))
