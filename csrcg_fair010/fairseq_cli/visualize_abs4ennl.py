...
def cluster_dict(cfg, trainer, task, epoch_itr):
    ...
    # added
    #selected_tokens = [l.strip().split()[0] for l in open('../entudata/devtest.nl.vocab.txt', 'r', encoding='utf-8')]
    #token_states_tgt = dict(((w, []) for w in selected_tokens))
    ...
    with torch.no_grad():
        for i_s, sample in enumerate(itr):  # delayed update loop
            ...
            for ib, l in enumerate(get_sample):
                # eos, punc
                for iw, w in enumerate(l.tolist()):
                    if w not in exwords:
                        w_str = trainer.model.encoder.dictionary[w]

                        if w_str in token_states:
                            token_states[w_str].append(state[ib, iw])
            '''

            # decoder
            state, _ = trainer.model.decoder(
            ...
            for ib, l in enumerate(sample['target']):
                # eos, punc
                for iw, w in enumerate(l.tolist()):
                    
                    if 55 <= w <= 30000:
                        w_str = trainer.model.decoder.dictionary[w]
                        if w_str in token_states_tgt:
                            if len(token_states_tgt[w_str]) <= 50000:
                                token_states_tgt[w_str].append(state[ib, iw])
                        else:
                            token_states_tgt[w_str] = [state[ib, iw]]
    import pickle as pck
    path = cfg.checkpoint.save_dir
    pck.dump((token_states, token_states_tgt), open('{}/token_states.pkl'.format(path), 'wb'))
    #pck.dump((token_states, token_states_tgt), open('{}/test_token_states.pkl'.format(path), 'wb'))
