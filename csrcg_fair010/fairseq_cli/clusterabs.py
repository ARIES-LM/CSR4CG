...
def cluster_dict(cfg, trainer, task, epoch_itr):
    ...
    packnum = trainer.model.args.pack_num
    ...
    trainer.model.set_batchmp(True)
    ...
    with torch.no_grad():
        for i_s, sample in enumerate(itr):  # delayed update loop
            ...
            # P B H
            # abs_states = encoder_out['pack']
            # abs_states = abs_states.cpu().numpy()
            # all_hiddens.append(abs_states)

            # packbottom.append(encoder_out['packed_before'][0].cpu().numpy())
            # packtop.append(encoder_out['packed_before'][-1].cpu().numpy())
            # packmid.append(encoder_out['packed_before'][2].cpu().numpy())

            # attn weights
            # print(sample['net_input']['src_tokens'].size())

            # list[] layers of B P L

            a_mean = sum(encoder_out['packed_before'][:-2]) / (len(encoder_out['packed_before']) - 2)

            # attn_ws.append(a_mean.cpu().numpy())

            # a = encoder_out['packed_before'][-1]
            #
            # # print(a.size(-1))
            #
            # if a.size(-1) > 7:
            #     for layer in range(len(encoder_out['packed_before'])):
            #         # B P L
            #         attw = encoder_out['packed_before'][layer]

            p = a_mean.unsqueeze(1).expand(-1, packnum, -1, -1)
            q = a_mean.unsqueeze(2).expand(-1, -1, packnum, -1)

            kl1 = F.kl_div(p.log(), q, reduction='none').sum(-1)
            kl2 = F.kl_div(q.log(), p, reduction='none').sum(-1)
            js = (kl1 + kl2 ) / 2
            ...
            js_accu += js
                ...
                # for si in range(a.size(0)):
                #     for i in range(packnum):
                #         for j in range(packnum):
                #             if i != j:
                #                 for layer in range(len(encoder_out['packed_before'])):

                # print('0 layer')
                # print(encoder_out['packed_before'][0][6])
                # print('1 layer')
                # print(encoder_out['packed_before'][1][6])
                # print('2 layer')
                # print(encoder_out['packed_before'][2][6])
                #
                # print('asdasdasda')
                #
                # print('0 layer')
                # print(encoder_out['packed_before'][0][16])
                # print('1 layer')
                # print(encoder_out['packed_before'][1][16])
                # print('2 layer')
                # print(encoder_out['packed_before'][2][16])
                # exit()
    ...
    # all_hiddens = np.concatenate(all_hiddens, 1)
    # np.save('{}/packbefore'.format(path), all_hiddens)

    # np.save('{}/absatts'.format(path), np.concatenate(attn_ws, 1))
    # np.save('{}/packtop'.format(path), np.concatenate(packtop, 1))
    # np.save('{}/topatt'.format(path), np.concatenate(packtop, 0))
