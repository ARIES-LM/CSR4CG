...
# added
@register_lr_scheduler("warmup_linear_decay", dataclass=InverseSquareRootLRScheduleConfig)
class WarmupLinearDecaySchedule(InverseSquareRootSchedule):
    def __init__(self, cfg: InverseSquareRootLRScheduleConfig, optimizer):
        super().__init__(cfg, optimizer)

        max_update = cfg.max_update

        warmup_end_lr = cfg.lr[0] if isinstance(cfg.lr, Collection) else cfg.lr
        if cfg.warmup_init_lr < 0:
            cfg.warmup_init_lr = 0 if cfg.warmup_updates > 0 else warmup_end_lr

        self.lr_step_decay = (warmup_end_lr - cfg.warmup_init_lr) / (max_update - cfg.warmup_updates)
