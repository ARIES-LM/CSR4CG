...
@register_model("transformer_bivq")
class TransformerModel(FairseqEncoderDecoderModel):
    ...
    def forward(
        ...
        # tgt_emb = self.embed_scale * self.decoder.embed_tokens(tgt_tokens)
        # tgt_emb += self.decoder.embed_positions(tgt_tokens)
        # tgt_emb = self.encoder.dropout_module(tgt_emb)
        ...
        if self.training and 'vqloss' in encoder_out:
            logtis, extra = decoder_out
            extra['vqloss'] = encoder_out['vqloss']
            decoder_out = logtis, extra
...
class TransformerEncoder(FairseqEncoder):
    ...
    # added
    def kmquantize(self, x, special, src_tokens):
        ...
        # tanh ffn
        # x_rec = torch.tanh(self.decfc1(quan_x))
        # x_rec= self.decfc2(x_rec)
        ...
        # ln / dp?
        # x = residual + quan_x
        # x = self.vq_layer_norm(residual + quan_x)
        # x = self.vq_layer_norm(residual + self.vqdp(quan_x))
...
@register_model_architecture("transformer_bivq", "transformer_bivq")
...
@register_model_architecture("transformer_bivq", "transformer_bivq_iwslt_de_en")
