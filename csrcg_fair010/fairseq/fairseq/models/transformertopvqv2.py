...
@register_model("transformer_vqtopv2")
...
class TransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        if vqtype == 'gumbel':
            ...
            # self.decfc1 = Linear(embed_dim, embed_dim*2)
            # self.decfc2 = Linear(embed_dim*2, embed_dim)
    ...
    def forward_scriptable(
        ...
        for ix, layer in enumerate(self.vqencoder):
            quan_x = layer(quan_x, encoder_padding_mask)

        vqoutput = self.kmquantize(quan_x, special_tokens, src_tokens)
        quan_x = vqoutput['x']
        ...
        quan_x = torch.tanh(self.decfc1(quan_x))
        quan_x = self.decfc2(quan_x)

        # T B H
        l2loss = F.mse_loss(quan_x, target, reduction='none')
        l2loss = l2loss.masked_fill(encoder_padding_mask.T.unsqueeze(-1), 0)
        l2loss = l2loss.sum() / (~encoder_padding_mask).float().sum()
        ...
        vqlosses = [kmloss, l2loss]
        # diverloss = diverloss / (self.num_layers - self.vqfroml + 1)
...
@register_model_architecture("transformer_vqtopv2", "transformer_vqtopv2")
...
@register_model_architecture("transformer_vqtopv2", "transformer_vqtopv2_iwslt_de_en")
