...
@register_model("transformer_vqx")
class TransformerModel(FairseqEncoderDecoderModel):
    ...
    def __init__(self, args, encoder, decoder):
        ...
        '''
        # added
        vqencdeclayers = args.encoder_layers
        self.vqencoder = nn.ModuleList([self.encoder.build_encoder_layer(args, i)  for i in range(vqencdeclayers)])

        self.encfc1 = Linear(embed_dim, embed_dim)
        self.encfc2 = Linear(embed_dim, embed_dim)
        '''
        ...
        '''
        self.vqdecoder = nn.ModuleList([self.encoder.build_encoder_layer(args, i)  for i in range(vqencdeclayers)])
        self.decfc1 = Linear(embed_dim, embed_dim)
        self.decfc2 = Linear(embed_dim, embed_dim)

        # freeze
        self.encoder.requires_grad = False
        self.decoder.requires_grad = False
        '''
        self.vq_group = args.vq_group

        # version 2
        if args.vq_dim < args.encoder_embed_dim:
            self.proj2x = Linear(args.vq_dim, args.encoder_embed_dim, False)

        # self.proj2x = nn.Linear(args.vq_dim, args.encoder_embed_dim*2)
        # self.proj2x2 = nn.Linear(args.encoder_embed_dim*2, args.encoder_embed_dim)
    ...
    def build_model(cls, args, task):
        ...
        # added
        decoder = cls.build_decoder(args, src_dict, encoder_embed_tokens)
    ...
    def forward(
        ...
        # P B H 
        packemb = encoder_out['encoder_out'][0][:self.args.packnum]
        quan_emb = packemb
        ...
        if self.args.vq_dim < self.args.encoder_embed_dim:
            quan_x = self.proj2x(quan_x)
        
        # quan_emb = self.layernorm_vq(self.proj2x(quan_x))
        # quan_emb = self.proj2x2(F.relu(self.proj2x(quan_x)))
        # quan_emb = self.layernorm_vq(quan_emb)
        ...
        logits, extra = decoder_out
        # B T H
        logits = logits[:, self.args.packnum:]

        extra["kmloss"] = q["kmeans_loss"]
        extra["codeppl"] = q["code_perplexity"]

        decoder_out = logits, extra
    ...
    def forward_hiddenrec(
        ...
        with torch.no_grad():
            ...
            ctxemb = encoder_out['encoder_out'][0]
            mask = encoder_out['encoder_padding_mask'][0]

        # x = self.vqdp(ctxemb)
        x = torch.zeros_like(ctxemb).copy_(ctxemb)
        for ix, layer in enumerate(self.vqencoder):
            x = layer(x, encoder_padding_mask=mask)
        
        # B T H
        x = x.transpose(0,1)

        quantization_mask = ~mask
        decinput = torch.zeros_like(x)
        ...
        decinput[quantization_mask] = quan_x
        x = decinput.transpose(0, 1)
        
        for ix, layer in enumerate(self.vqdecoder):
            x = layer(x, encoder_padding_mask=mask)
        
        x = torch.tanh(self.decfc1(x))
        x = self.decfc2(x)

        # loss
        decoder_out = {'kmloss':kmloss, 'codeppl':codeppl}

        # T B H
        l2loss = F.mse_loss(x, ctxemb, reduction='none')
        l2loss = l2loss.masked_fill(mask.T.unsqueeze(-1), 0)
        l2loss = l2loss.sum() / (~mask).float().sum()
        decoder_out['l2loss'] = l2loss
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def forward(
        ...
        **kwargs
        ...
        if encoder_out is not None and 'diverloss' in encoder_out:
            extra['diverloss'] = encoder_out['diverloss']
...
@register_model_architecture("transformer_vqx", "transformer_vqx")
...
@register_model_architecture("transformer_vqx", "transformer_vqx_iwslt_de_en")
