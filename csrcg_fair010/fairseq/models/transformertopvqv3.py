...
@register_model("transformer_vqtopv3")
class TransformerModel(FairseqEncoderDecoderModel):
    ...
    def add_args(parser):
        ...
        # for vq
        parser.add_argument('--vq-pretrain', type=int, default=1)
        ...
        parser.add_argument('--vq-froml', type=int, default=1)
        parser.add_argument('--vq-ema', type=float, default=0, help='ema decay')
        parser.add_argument('--vq-cos', type=int, default=0, help='use cosine distance')
        # parser.add_argument('--intersteps', type=int, default=0)
        parser.add_argument('--vq-minvar', type=float, default=1000)
        parser.add_argument('--vq-orth', type=float, default=0.0)
    ...
    def forward(
        ...
        # added cl during training
        if self.training:
            _, extra = decoder_out
            extra['encoder_hidden'] = encoder_out['h_beforevq']
...
class TransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        if vqtype == 'gumbel':
            ...
            # self.decfc1 = Linear(vq_dim, embed_dim)
            ...
            # self.comb = Linear(embed_dim+vq_dim, embed_dim)
    ...
    # added
    def kmquantize(self, x, special, src_tokens, bibranch=False):
        residual = x

        # B T H
        x = x.transpose(0, 1)

        quantization_mask = ~special
        ...
        kmloss = 0.0
        x_codeemb = []
        for quantizer_i in self.input_quantizer:
            ...
            kmloss += loss
            if self.training:
                quantizer_i.codeuse += q["codeuse"].float()

            annotation = x.new_zeros(x.size(0), x.size(1), quan_x.size(-1))

            annotation[quantization_mask] = quan_x
            # T B Z
            quan_x = annotation.transpose(0, 1)
        ...
        # loss
        # out = {'kmloss':kmloss, 'codeppl':codeppl, 'idx':idx}
        
        with torch.no_grad():
            # for d in dist:
            #     print(d)
            #     print(d.var().item(), d.min().item(), d.max().item(), d.mean().item())
            #     print()
            # exit()
            #var_lt_mean = var_lt_mean.float().unsqueeze(1)

            distvar = dist.var(dim=1)
            # N*G
            mean_var = distvar.mean()
            # var_lt_mean = distvar < mean_var
            # if self.training:
            #     print('dist mean, min, max, var_lt_mean', distvar.mean().item(), distvar.min().item(), distvar.max().item(),
            #           (var_lt_mean.sum().float() / var_lt_mean.size(0)).item())

        # only quanx
        # linear combination
        # x = self.comb(torch.cat((residual, quan_x), -1))

        if self.args.vq_pretrain:
            x = quan_x + self.embed_positions(src_tokens).transpose(0, 1)
            x = self.vq_layer_norm(x)
            # x = quan_x
        else:
            var_lt = distvar < self.args.vq_minvar

            quan_x = quan_x + self.embed_positions(src_tokens).transpose(0, 1)
            quan_x = self.vq_layer_norm(quan_x)

            # B T G
            mask = annotation.new_ones(x.size(0), x.size(1), self.args.vq_group)
            mask[quantization_mask] = var_lt.reshape(-1, self.args.vq_group).float()
            # T B G 1
            mask = mask.transpose(0, 1).unsqueeze(-1)

            tsize, bsize, hsize = residual.size()
            #print('2', self.num_updates)
            # print(var_lt_mean.size(), x.size(), quan_x.size())
            x = mask * residual.view(tsize, bsize, self.args.vq_group, -1) + \
                (1 - mask) * quan_x.view(tsize, bsize, self.args.vq_group, -1)
            x = x.view(tsize, bsize, -1)
    ...
    def forward_scriptable(
        ...
        # added
        special_tokens = torch.zeros_like(encoder_padding_mask)
        for j in range(4):
            special_tokens = special_tokens | src_tokens.eq(j)

        vqoutput = self.kmquantize(x, special_tokens, src_tokens)
        x = vqoutput['x']
        kmloss += vqoutput['kmloss']

        # idx = vqoutput['idx']
        # src_code = torch.zeros_like(src_tokens).fill_(-1)
        # src_code[~encoder_padding_mask] = idx

        # for ib in range(5):
        # print(src_tokens[ib].tolist())
        # print(src_code[ib].tolist())

        vqlosses = [kmloss]
        if 'orth_loss' in vqoutput:
            vqlosses.append(vqoutput['orth_loss'])
...
@register_model_architecture("transformer_vqtopv3", "transformer_vqtopv3")
...
@register_model_architecture("transformer_vqtopv3", "transformer_vqtopv3_iwslt_de_en")
