...
class TransformerModel(FairseqEncoderDecoderModel):
    ...
    def build_embedding(cls, args, dictionary, embed_dim, path=None):
        ...
        emb = Embedding(num_embeddings, embed_dim, padding_idx, None)
    ...
    def id2str(self, x, vocab):
        return ' '.join([vocab[w] for w in x.tolist()])

    def _sent_rep(self, h, mask):
        h = h.masked_fill(mask.unsqueeze(-1), 0)
        h, _ = h.max(1)

        # if not hasattr(self.args, 'sentrep') or self.args.sentrep == 'mean':
        #     h = h.masked_fill(mask.unsqueeze(-1), 0)
    ...
    def forward_distance(
        ...
        # B T H
        # ench = encoder_out['encoder_out'][0].transpose(0, 1)
        ench = encoder_out['encoder_states'][1].transpose(0, 1)

        # "encoder_states": encoder_states,  # List[T x B x C]

        # B T
        enc_mask = encoder_out['encoder_padding_mask'][0]
        enc_sentrep = self._sent_rep(ench, enc_mask)

        #anchor_idx = random.randint(0, enc_mask.size(0))
        anchor_idx = 1
        anchor_src = enc_sentrep[anchor_idx:anchor_idx+1]
        anchor_cos_other = F.cosine_similarity(anchor_src.float(), enc_sentrep.float())

        print('anchor', anchor_idx)
        print(self.id2str(src_tokens[anchor_idx], self.encoder.dictionary))

        print('src top', topk)
        src_topk_score, src_topk_idx = torch.topk(anchor_cos_other, topk, dim=0, largest=True, sorted=True)

        # for j, js in zip(src_topk_idx.tolist(), src_topk_score.tolist()):
        #     print(j, js, self.id2str(src_tokens[j], self.encoder.dictionary))

        print(src_topk_score.tolist())
        print(src_topk_idx.tolist())

        dech, extra = decoder_out
        dec_mask = prev_output_tokens.eq(self.decoder.padding_idx)
        dec_sentrep = self._sent_rep(dech, dec_mask)

        anchor_tgt = dec_sentrep[anchor_idx:anchor_idx+1]
        tgt_anchor_cos_other = F.cosine_similarity(anchor_tgt.float(), dec_sentrep.float())
        tgt_topk_score, tgt_topk_idx = torch.topk(tgt_anchor_cos_other, topk, dim=0, largest=True, sorted=True)

        print('tgt top', topk)
        # print(self.id2str(prev_output_tokens[anchor_idx], self.decoder.dictionary))
        # for j, js in zip(tgt_topk_idx.tolist(), tgt_topk_score.tolist()):
        #     print(j, js, self.id2str(prev_output_tokens[j], self.decoder.dictionary))

        print(tgt_topk_score.tolist())
        print(tgt_topk_idx.tolist())
    ...
    def forward(
        ...
        **kwargs
        ...
        #self.forward_distance(src_tokens, src_lengths, prev_output_tokens)
        #exit()
...
class TransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        # added
        if hasattr(args, 'universal') and args.universal:
...
class TransformerLinDecoder(TransformerDecoder):
    def __init__(
        self,
        args,
        dictionary,
        ...
        super().__init__(args, dictionary, embed_tokens,
                    no_encoder_attn, output_projection,)
    ...
    def forward(
        ...
        **kwargs
        ...
        if self.training:
            ...
            xs = []
            for t in torch.arange(0, prev_output_tokens.size(1)):

                tokens = prev_output_tokens[:, t:t+1]
...
@register_model("transformer_lin")
class TransformerLin(TransformerModel):
    def __init__(self, args, encoder, decoder):
        super().__init__(args, encoder, decoder)

    @classmethod
    def build_decoder(cls, args, tgt_dict, embed_tokens):
        return TransformerLinDecoder(
            args,
            tgt_dict,
            embed_tokens,
            no_encoder_attn=getattr(args, "no_cross_attention", False),
...
@register_model_architecture("transformer_lin", "transformer_lin_iwslt_de_en")
def transformer_lin_iwslt_de_en(args):
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 512)
    args.encoder_ffn_embed_dim = getattr(args, "encoder_ffn_embed_dim", 1024)
    args.encoder_attention_heads = getattr(args, "encoder_attention_heads", 4)
    args.encoder_layers = getattr(args, "encoder_layers", 6)
